...
void main() {
  ...
  group('RetroAchievements Widgets', () {
    ...
    testWidgets('PlayerRetroAchievementsPage renders stats and single-scrollable game progress', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(
            home: SDTFScope(
              child: PlayerRetroAchievementsPage(),
            ),
          ),
        ),
      );
      ...
      expect(
        find.byWidgetPredicate(
          (w) => w is Text && (w.data?.contains('1,870') == true || w.textSpan?.toPlainText().contains('1,870') == true),
        ),
        findsOneWidget,
      );
      expect(
        find.byWidgetPredicate(
          (w) => w is Text && (w.data?.contains('#57,401') == true || w.textSpan?.toPlainText().contains('#57,401') == true),
        ),
        findsOneWidget,
      );
  ...
  group('RetroAchievements Game Matching & Caching', () {
    test('GameRetroAchievementsRepo saves and retrieves game entries', () async {
      final db = AppDatabase(NativeDatabase.memory());
    ...
    test('computeRomMd5 hashes raw files and .zip archives correctly', () async {
      ...
      try {
        ...
        final archive = Archive()
          ..addFile(ArchiveFile('game.sfc', 20, 'SUPER_MARIO_TEST_ROM'.codeUnits));
        final zipBytes = ZipEncoder().encode(archive);
        await zipFile.writeAsBytes(zipBytes);

        final zipMd5 = await computeRomMd5(zipFile.path);
        expect(zipMd5?.primaryHash, rawMd5.primaryHash); // Inner ROM MD5 should match!
      } finally {
        await tempDir.delete(recursive: true);
      }
    });

    test('resolveGameRetroAchievements caches result and matches system', () async {
      final db = AppDatabase(NativeDatabase.memory());
      final repo = GameRetroAchievementsRepo(db);
      final tempDir = await Directory.systemTemp.createTemp('ra_resolve_');

      try {
        final snesDir = Directory('${tempDir.path}/snes')..createSync(recursive: true);
    ...
    test('resolveGameRetroAchievements identifies GameCube disc ISO correctly', () async {
      final db = AppDatabase(NativeDatabase.memory());
      final repo = GameRetroAchievementsRepo(db);
      final tempDir = await Directory.systemTemp.createTemp('ra_gc_resolve_');

      try {
        final gcDir = Directory('${tempDir.path}/gc')..createSync(recursive: true);
        ...
        final apploaderInfo = ByteData.sublistView(gcDisc, baseHeaderSize + 0x14, baseHeaderSize + 0x1C);
    ...
    testWidgets('GameAchievementsPage renders achievements list and user progress', (tester) async {
      final db = AppDatabase(NativeDatabase.memory());
      final repo = GameRetroAchievementsRepo(db);
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            selectedGameProvider('snes').overrideWith(() => _MockSelectedGameNotifier('snes', game)),
            gameRetroAchievementsDetailsProvider(228).overrideWith(
              (ref) async => mockProgress,
            ),
          ],
          child: MaterialApp(
            home: GameAchievementsPage(
              system: 'snes',
              hash: game.hash,
            ),
          ),
    ...
    test('RetroAchievementsCacheRepo saves, retrieves, and invalidates API cache entries', () async {
      final db = AppDatabase(NativeDatabase.memory());
